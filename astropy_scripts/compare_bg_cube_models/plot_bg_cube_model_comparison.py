...
# convert observation groups binning definition "a_la_michi" to "a_la_gammapy"

list_obs_group_axis = [ObservationGroupAxis('ALT', altitude_edges, 'bin_edges'),
                       ObservationGroupAxis('AZ', azimuth_edges, 'bin_edges')]
obs_groups_a_la_michi = ObservationGroups(list_obs_group_axis)
print("Observation groups 'a_la_michi':")
print(obs_groups_a_la_michi.obs_groups_table)
if SAVE:
    outfile = 'bg_observation_groups_a_la_michi.ecsv'
    print('Writing {}'.format(outfile))
    obs_groups_a_la_michi.write(outfile)

# lookup table: equivalences in group/file naming "gammapy" <-> "a_la_michi"
# 3 columns: GROUP_ID, ALT_ID, AZ_ID
# 28 rows: 1 per GROUP_ID

lookup_obs_groups_a_la_michi = Table()
n_cols = 1 + len(list_obs_group_axis)
n_rows = obs_groups_a_la_michi.n_groups
lookup_obs_groups_a_la_michi['GROUP_ID'] = np.zeros(n_rows, dtype=np.int)
lookup_obs_groups_a_la_michi['ALT_ID'] = np.zeros(n_rows, dtype=np.int)
lookup_obs_groups_a_la_michi['AZ_ID'] = np.zeros(n_rows, dtype=np.int)
...
for alt_id in np.arange(len(altitude_edges) - 1):
    for az_id in np.arange(len(azimuth_edges) - 1):
        lookup_obs_groups_a_la_michi['GROUP_ID'][count_groups] = count_groups
        lookup_obs_groups_a_la_michi['ALT_ID'][count_groups] = alt_id
        lookup_obs_groups_a_la_michi['AZ_ID'][count_groups] = az_id
        count_groups += 1

print("lookup table:")
print(lookup_obs_groups_a_la_michi)

if SAVE:
    outfile = 'lookup_obs_groups_a_la_michi.ecsv'
    print('Writing {}'.format(outfile))
...
#       "a_la_michi"; otherwise I need to make a generic cartesian
...
def plot_bg_cube_model_comparison():
    ...
    if binning_format1 == 'a_la_michi':
        observation_groups = obs_groups_a_la_michi
    else:
